...
class UNOptimizer(OptimizerBase):
    """
    ...
    def __init__(self, dbf, phase_models=None, scheduler=None):
        super(UNOptimizer, self).__init__(dbf)
    ...
    #@staticmethod
    def UN_prior_transform(self, quantile_cube):
        symbol_list = self.ctx.get('symbols_to_fit')
        prior_rvs = self.ctx.get('prior_rvs', [rv_zero() for _ in range(len(symbol_list))])
        lnprior_multivariate = np.array([rv.ppf(quantile) for rv, quantile in zip(prior_rvs, quantile_cube)])
        return lnprior_multivariate
